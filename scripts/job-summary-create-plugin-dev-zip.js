...
if (rawMetaProvided) {
  try {
    ...
    assertField(
      preferredVersions,
      "php",
      "playground.preferredVersions.php required"
    );
    assertField(
      preferredVersions,
      "wp",
      "playground.preferredVersions.wp required"
    );
    playgroundSupported = true;
  } catch (e) {
    console.warn(
      "[summary] Playground metadata incomplete; no playground URL will be created:",
      e.message
    );
  }
}
